"""Tests for visualizer.py — figure generation.

These are smoke tests: matplotlib output is best validated visually, but an
automated check that all three figures are generated, non-empty, and don't
crash on edge cases (e.g. a class missing from one CV fold) still catches
real regressions — environment/version mismatches, KeyErrors from result
dicts, and shape mismatches in plotting code.
"""
...
def results_and_dir(synthetic_beats_labels, tmp_path_factory):
    ...
    _, results = model_module.train_and_evaluate(X, y, feature_names, output_dir=tmp_dir)
...
def test_generate_report_does_not_pollute_real_results_dir(results_and_dir, tmp_path):
    """Regression guard: generate_report must respect output_dir and never
    fall back to writing inside the package directory during tests."""
    from visualizer import generate_report
